...
/*
 * Файловый менеджер осуществляет операции с файлами и директориями.
 * Оперирует с виртуальным деревом каталогов и файлов, каждый файл или директория имеют
 * имя, состоящее из латинских прописных и заглавных букв, цифр и символов '_' и '.'.
 * Длина имени ограничена 32 символами. Файлы также имеют размер (в байтах).
 *
 ...
 * Для идентификации файла или директории используется путь, состоящий из имен вложенных
 * директорий, содержащих данный файл, и знаков '/', например /dir1/dir2/file.txt.
 * 
 * Любой файл или директория доступны по относительным и абсолютным путям.
 * Относительный путь считается относительно текущей директории, абсолютный путь начинается
 * с корневой директории, т. е. '/', например /my_dir1/my_dir2/file.txt и my_dir2/file.txt
 * указывают на один и тот же файл при условии, что /my_dir1 - текущая директория.
 ...
 *
 * В связи с этим запрещается создавать файлы или директории с именами '.' и '..'.
 * В случае попытки создать такой файл или директорию менеджер должен выдавать ошибку.
 *
...
{
    /*
     * Создает файловый менеджер с указанным размером диска.
     ...
     * Возвращаемое значение: 1 - успех, 0 - неудача (например, менеджер не был создан).
     */
    int (*destroy)();
    ...
    /*
     * Проверяет, является ли указанное имя директорией.
     ...
     * Если установлен флаг dir_first, то сначала выводит все поддиректории, а потом файлы.
     ...
     * Если хотя бы один файл или директория из списка или шаблона * не существует,
